...
if (document.getElementById('section-list')) {
    ...
    async function loadDashboard(postSubmission = false) {
        console.log('loadDashboard triggered at', new Date().toISOString());
        ...
        try {
            ...
            `;
                ...
                card.onclick = () => loadSection(card.dataset.section); // Dataset use karna safe hai
            ...
            if (submitButton) {
                ...
                submitButton.onclick = async () => {
                    ...
                    try {
                        ...
                        // Clear completion status and prevent ticks on reload
                        completionStatus = {};
                        ...
                        // Reload dashboard without re-populating completion status
                        await loadDashboard(true); // Pass flag to indicate post-submission state
                    } catch (err) {
    ...
    async function loadSection(section) {
        console.log('loadSection triggered for', section, 'at', new Date().toISOString());
...
async function clearAllSectionData() {
    ...
    try {
        await fetch(`${API_BASE_URL}/api/clear-sections`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
    } catch (err) {
