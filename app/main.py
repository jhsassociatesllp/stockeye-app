...
## OUTLOOK SMTP EMAIL SENDING
# @app.post("/api/send-email")
# async def send_email(
#     from_email: str = Form(...),
#     to_email: str = Form(...),
#     generate_from_data: bool = Form(False),
#     attachment: UploadFile = File(None),
#     emp_id: str = Depends(get_current_user)
# ):
#     """
#     Send email via Outlook SMTP with either uploaded file or generated report.
#     """
#     try:
#         today = datetime.now(timezone.utc).date().isoformat()
#         logger.info(f"Email send request from {from_email} → {to_email} by {emp_id}")

#         # === STEP 1: Get / Generate the document ===
#         if attachment:
#             # If user uploaded a file
#             file_data = await attachment.read()
#             file_name = attachment.filename
#         else:
#             # Generate audit report Word file dynamically
#             audit_data = await audit_data_collection.find_one({"user_id": emp_id, "date": today})
#             if not audit_data:
#                 audit_data = await temp_audit_data_collection.find_one({"user_id": emp_id, "date": today})
#             if not audit_data:
#                 return JSONResponse(
#                     content={"message": "No audit data found for this user/date", "success": False},
#                     status_code=status.HTTP_404_NOT_FOUND
#                 )
...
#                         if q.get("remarks"):
#                             doc.add_paragraph(f"Remarks: {q.get('remarks', '')}")
#                 else:
...
#                     img_bytes = io.BytesIO(base64.b64decode(img_data))
#                     doc.add_heading("Signature", level=2)
#                     doc.add_paragraph("Signature captured during audit:")
#                     doc.add_picture(img_bytes, width=Inches(2.5))
#                 except Exception as e:
#                     doc.add_paragraph(f"⚠️ Could not add signature: {e}")
...
#                     doc.add_picture(img_bytes, width=Inches(3.5))
#                 except Exception as e:
#                     doc.add_paragraph(f"⚠️ Could not add photo: {e}")
...
#             file_name = f"audit_{emp_id}_{today}.docx"
...
#         msg["From"] = from_email
#         msg["To"] = to_email
#         msg["Cc"] = from_email  # optional, CC to sender
#         msg.set_content(f"""
#             Dear Auditor Manager,

#             Please find attached the audit report for {today} submitted by {emp_id}.
...
#         # === STEP 3: Send via Outlook SMTP ===
#         smtp_server = "smtp.office365.com"
#         smtp_port = 587
#         smtp_user = os.getenv("MAIL_USERNAME")  # e.g. your@outlook.com
#         smtp_pass = os.getenv("MAIL_PASSWORD")  # app password or actual password

#         logger.info(f"Connecting to Outlook SMTP: {smtp_server}:{smtp_port} as {smtp_user}")
...
#         return JSONResponse(
#             content={"message": "Email sent successfully via Outlook SMTP", "success": True},
#             status_code=status.HTTP_200_OK
#         )
...
#         return JSONResponse(
#             content={"message": f"Failed to send email: {str(e)}", "success": False},
#             status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
#         )
...
# ):
#     """
#     Send audit report as Word attachment via Gmail SMTP.
#     Generates Word file same as /api/export-word.
#     """
#     try:
#         today = datetime.now(timezone.utc).date().isoformat()
#         logger.info(f"Email send request from {from_email} → {to_email} by {emp_id}")

#         # === STEP 1: Prepare or Generate Word File ===
#         if attachment:
#             # if user uploaded manual file
#             file_data = await attachment.read()
#             file_name = attachment.filename

#         else:
#             # fetch today's audit data
#             audit_data = await audit_data_collection.find_one({"user_id": emp_id, "date": today})
#             if not audit_data:
#                 return JSONResponse(
#                     content={"message": "No audit data found for today", "success": False},
#                     status_code=404
#                 )
...
#             if not all_completed:
#                 return JSONResponse(
#                     content={"message": "Not all sections are completed before sending email", "success": False},
#                     status_code=400
#                 )
...
#         # === STEP 3: Send using Gmail SMTP ===
#         smtp_server = "smtp.gmail.com"
#         smtp_port = 587
#         smtp_user = os.getenv("MAIL_USERNAME")  # e.g. your@gmail.com
#         smtp_pass = os.getenv("MAIL_PASSWORD")  # App password

#         logger.info(f"Connecting to Gmail SMTP as {smtp_user}")
#         with smtplib.SMTP(smtp_server, smtp_port) as smtp:
#             smtp.ehlo()
#             smtp.starttls()
#             smtp.login(smtp_user, smtp_pass)
...
#         return JSONResponse(
#             content={"message": "Email sent successfully via Gmail SMTP", "success": True},
#             status_code=200
#         )
