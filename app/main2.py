...
async def get_sections(emp_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        completion_status = {
            ...
            "stock_count": audit["completion_status"].get("stock_count", False) if audit and "completion_status" in audit else False,
        }
        response = base_response.copy()
        response.update({
...
# ============= UPLOAD DATA ENDPOINT (Managers Only) =============
@app.post("/api/upload-items")
async def upload_items(
    file: UploadFile = File(...),
    emp_id: str = Depends(get_current_user)
):
    """Upload Excel/CSV file containing Item Code and Item Name"""
    try:
        # Check file extension
        if not (file.filename.endswith('.xlsx') or file.filename.endswith('.csv')):
            return JSONResponse(
                {"message": "Only Excel (.xlsx) or CSV files are allowed", "success": False},
                status_code=400
            )
        ...
        try:
            ...
            return JSONResponse(
                {"message": f"Could not parse file: {str(parse_error)}", "success": False},
                status_code=400
            )
        ...
        # Try to find Item Code and Item Name columns in the CURRENT header first
        item_code_col = None
        item_name_col = None
        
        # Search in current column names
        for col in df.columns:
            col_str = str(col).lower().strip()
            if 'item code' in col_str and not item_code_col:
                item_code_col = col
            if 'item name' in col_str and not item_name_col:
                item_name_col = col
        
        # If columns found in header, use them directly
        if item_code_col and item_name_col:
            logger.info(f"Found columns in header: {item_code_col}, {item_name_col}")
        else:
            # If not found in header, search for them in the data rows
            logger.info("Columns not in header, searching in data rows...")
            header_row_idx = None
            ...
            for idx in range(min(20, len(df))):  # Check first 20 rows max
                ...
                if 'item code' in row_str and 'item name' in row_str:
                    header_row_idx = idx
            ...
            if header_row_idx is None:
                ...
                return JSONResponse(
                    {"message": f"Could not find 'Item Code' and 'Item Name' columns. Available columns: {available_cols}", "success": False},
                    status_code=400
                )
            
            # Set the found row as header
            new_header = [str(col).strip() for col in df.iloc[header_row_idx]]
            df = df.iloc[header_row_idx + 1:].copy()
            df.columns = new_header
            df.reset_index(drop=True, inplace=True)
            
            logger.info(f"New columns after setting header: {df.columns.tolist()}")
            
            # Find column names again
            for col in df.columns:
                col_str = str(col).lower().strip()
                if 'item code' in col_str and not item_code_col:
                    item_code_col = col
        ...
        if not item_code_col or not item_name_col:
            available_cols = ', '.join([str(c) for c in df.columns])
            return JSONResponse(
                {"message": f"Could not identify 'Item Code' and 'Item Name' columns. Available columns: {available_cols}", "success": False},
                status_code=400
            )
        
        logger.info(f"Using columns: Item Code = '{item_code_col}', Item Name = '{item_name_col}'")
        ...
        for idx, row in df.iterrows():
            try:
                item_code_val = row[item_code_col]
                item_name_val = row[item_name_col]
                
                # Convert to string and handle NaN
                if pd.isna(item_code_val):
                    item_code = ""
                else:
                ...
                if pd.isna(item_name_val):
                    ...
                    item_name = str(item_name_val).strip()
                
                # Skip empty, nan, or invalid rows
                if (item_code and 
                    item_code.lower() not in ['nan', 'none', '', 'item code'] and 
                    item_name and 
                    item_name.lower() not in ['nan', 'none', '', 'item name']):
        ...
        if not items:
            return JSONResponse(
                {"message": f"No valid items found in the file. Skipped {skipped} invalid rows.", "success": False},
                status_code=400
            )
        
        logger.info(f"Extracted {len(items)} valid items, skipped {skipped} rows")
        
        # Clear existing items and insert new ones
        item_master_collection.delete_many({})
        item_master_collection.insert_many(items)
        
        logger.info(f"Uploaded {len(items)} items by {emp_id}")
        
        return JSONResponse(
            {
                "message": f"Successfully uploaded {len(items)} items" + (f" (skipped {skipped} invalid rows)" if skipped > 0 else ""),
                "success": True,
                "data": {
                    "count": len(items),
                    "skipped": skipped,
                    "sample_items": [{"item_code": item["item_code"], "item_name": item["item_name"]} for item in items[:5]]
                }
            },
            status_code=200
        )
        ...
        return JSONResponse(
            {"message": f"Failed to upload items: {str(e)}", "success": False},
            status_code=500
        )

# ============= GET ITEMS FOR STOCK COUNT =============
@app.get("/api/get-items")
async def get_items(
    search: str = Query(None),
    emp_id: str = Depends(get_current_user)
):
    """Get all items for stock count with optional search"""
    try:
        # Build query
        query = {}
        if search:
        ...
        # Get items
        items = list(item_master_collection.find(query, {"_id": 0, "item_code": 1, "item_name": 1}))
        ...
        if audit and "stock_count_data" in audit:
            for item in audit["stock_count_data"]:
                stock_count_data[item["item_code"]] = {
        ...
        for item in items:
            ...
            if item_code in stock_count_data:
                item.update(stock_count_data[item_code])
            else:
                item["physical_amount"] = ""
                item["remarks"] = ""
        ...
        return JSONResponse(
            {"message": f"Failed to get items: {str(e)}", "success": False},
            status_code=500
        )
...
async def save_stock_count_item(
    request: Request,
    emp_id: str = Depends(get_current_user)
):
    """Save individual stock count item"""
    try:
        body = await request.json()
        ...
        # Initialize stock_count_data if not exists
        ...
        for item in audit["stock_count_data"]:
            if item["item_code"] == item_code:
                item["physical_amount"] = physical_amount
        ...
        return JSONResponse(
            {
                "message": "Stock count item saved successfully",
                "success": True
            },
            status_code=200
        )
        ...
        return JSONResponse(
            {"message": f"Failed to save: {str(e)}", "success": False},
            status_code=500
        )
...
async def submit_stock_count(emp_id: str = Depends(get_current_user)):
    """Submit stock count - mark as complete"""
    try:
        today = datetime.now(timezone.utc).date().isoformat()
        audit = temp_audit_data_collection.find_one({"user_id": emp_id, "date": today})
        ...
        if "stock_count_data" not in audit or len(audit["stock_count_data"]) == 0:
            return JSONResponse(
        ...
        return JSONResponse(
            {"message": f"Failed to submit: {str(e)}", "success": False},
            status_code=500
        )